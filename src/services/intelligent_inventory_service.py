...
from src.services.adaptive_forecasting_service import adaptive_forecast, get_sku_classification_metadata
from src.inventory.reorder_point import compute_reorder_decision
from src.inventory.business_constraints import (
...
from src.uncertainty.dynamic_sigma import compute_rolling_forecast_error, compute_dynamic_safety_stock
from src.uncertainty.prediction_intervals import (
...
class IntelligentInventoryService:
    ...
    def __init__(self, model=None, error_window_days: int = 30):
        self.model = model
    ...
    def get_intelligent_reorder_decision(
        ...
        if forecast_history is not None and len(forecast_history) > 0:
            try:
                ...
                print(f"Warning: Could not compute prediction intervals for {sku}: {e}")
        ...
        if forecast_history is not None and len(forecast_history) > 0:
            try:
                ...
                # Fallback to traditional method if error calculation fails
                print(f"Warning: Could not compute dynamic safety stock for {sku}: {e}")
    ...
    def batch_reorder_decisions(
        ...
        for sku, data in sku_data.items():
            try:
                decision = self.get_intelligent_reorder_decision(
                    ...
                    supplier_constraints=data.get("supplier_constraints")
                )
        ...
        # Apply budget optimization if provided
